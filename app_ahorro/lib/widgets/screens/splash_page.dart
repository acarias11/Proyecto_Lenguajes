...
class _SplashPageState extends State<SplashPage> {
  Future<void> _validateUser(BuildContext context) async {
    ...
    if (userId.toString() == null) {
      ...
      final Usuario? user = users.firstWhereOrNull((u) => u.userId.toString() == userId.toString());
      final List<Cuenta> cuentas = await DBHelper.queryCuentas();
      final Cuenta? cuenta = cuentas.firstWhereOrNull((c) => user?.userId.toString() == c.userid.toString());

      if (cuenta.toString() == null || user.toString() == null) {
